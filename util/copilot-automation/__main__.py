...
def build_json_prompt(issue_number, issue_id):
    ...
    return json.dumps({
        "content": (
            ...
            "The result ID (in square brackets) should always be in the form [blogs.] (make sure it's plural - blogs, NOT blog) where what follows after the period is the site domain part from the Site URL section without the protocol, and the periods are omitted entirely. "
            ...
            "The [blogs.] part should be in lowercase and ALWAYS there. The period separating it from the rest should ALWAYS be there. "
            ...
            "The TOML content should NOT contain any properties other than `title`, `url`, `favicon`, and `updated`. `updated` should be the current ISO-8601 date WITHOUT timezone (just Z at the end). If other properties are present, the results will be considered incorrect. "
            "In the TOML embedded in the JSON make sure to properly escape all characters that need to be escaped."
